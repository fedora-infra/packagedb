...
try:
    from urlparse import parse_qs # introduced in 2.6
except:
...
def remove_csrf_token(url):
    """Safe way to remove _csrf_token from querystring
    """
    url = urlparse(url)
    q = parse_qs(url[4])
    if '_csrf_token' in q:
        del q['_csrf_token']
    query = '&'.join(['%s=%s' % (a[0], ','.join(a[1])) for a in q.iteritems()])
    url_updated = list(url)
    url_updated[4] = query
    return urlunparse(url_updated)
