...
from pkgdb.lib.utils import admin_grp, \
        critpath_grps, fas, get_bz, newpkger_grp, \
        pkger_grp, provenpkger_grp, LOG, STATUS
...
class PackageDispatcher(controllers.Controller):
    ...
    def _create_or_modify_group_acl(self, pkg_listing, group_name, new_acl,
        ...
        :arg statusname: Status we're setting the ACL to.
        '''
        # Create the ACL
        change_group = None
    ...
    def _set_owner(self, pkg_listing, owner):
        ...
        if owner['username'] != 'orphan':
            ...
            pkg_listing.statuscode = STATUS['Approved']
            ...
            pkg_listing.statuscode = STATUS['Orphaned']
            ...
            if pkg.owner != 'orphan':
                try:
                    ...
                    return dict(status=False,
                                message=_('specified owner %(owner)s'
                ...
                try:
                    ...
                    return dict(status=False, 
                        message=_('Unable to retire/unretire package:'
                                  ' %(err)s') % {'err': e})
    ...
    def set_retirement(self, pkg_name, collectn, collectn_version, retirement):
        ...
        if (retirement == 'Retire'):
               ...
               (pkg.statuscode == STATUS['Orphaned'] or
                ...
                if pkg.owner != 'orphan':
                    ...
                    try:
                        ...
                        return dict(status=False, 
                            message=_('Unable to retire package: %(err)s') %
                                    {'err': e})
            ...
            return dict(status=False,
                        message=_('Invalid value for retirement.  Acceptable'
                                  ' values are "Retire" and "Unretire".  Value'
                                  ' received is %s') % (pkg_name))
    ...
    def set_acl_status(self, pkgid, person_name, new_acl, statusname):
        ...
        if statusname not in ('Denied', 'Obsolete'):
            try:
                _acl_can_be_held_by_user(new_acl, user)
            except AclNotAllowedError, e:
                return dict(status=False, message=str(e))

        person_acl = _create_or_modify_acl(pkg, person_name, new_acl,
                                           statusname)
    ...
    def toggle_acl_request(self, container_id):
        ...
        if acl_status != 'Obsolete':
            ...
            try:
                _acl_can_be_held_by_user(acl_name)
            except AclNotAllowedError, e:
                return dict(status=False, message=str(e))

        # Assign person to package
        person_acl = _create_or_modify_acl(pkg_listing,
                identity.current.user_name, acl_name, acl_status)
    ...
    def add_package(self, package, owner, summary):
        ...
        try:
            _acl_can_be_held_by_user('owner', person)
        except AclNotAllowedError, e:
            return dict(status=False, message=str(e))
        ...
        # Create the package
        pkg = Package(package, summary, STATUS['Approved'])
        try:
            session.flush() #pylint:disable-msg=E1101
        except SQLError, e:
        ...
        pkg_listing = pkg.create_listing(devel_collection, person['username'],
                'Approved', author_name = identity.current.user_name)
        try:
            session.flush() #pylint:disable-msg=E1101
        except SQLError, e:
            return dict(status=False, message=_('Unable to create'
        ...
        pkg_log = PackageLog(
                identity.current.user_name, STATUS['Added'],
                pkg_log_msg)
        ...
        logs.append(pkg_log_msg)
        pkg_log = PackageLog(
                identity.current.user_name, STATUS['Approved'],
                pkg_log_msg)
        pkg_log.package = pkg
    ...
    def edit_package(self, package, **changes):
        ...
        if 'summary' in changes:
            ...
            log = PackageLog(identity.current.user_name, STATUS['Modified'],
                    log_msg)
            log.package = pkg
            pkg_log_msg = log_msg
            ...
            for collection_name in collection_data:
                ...
                try:
                    ...
                    if owner_name == 'orphan':
                        ...
                        statusname = 'Approved'
                    pkg_listing = pkg.create_listing(collection, owner_name,
                            statusname,
                    ...
                    for group in (provenpkger_grp,):
                        #pylint:disable-msg=E1101
                        changed_acls.append(GroupPackageListingAcl.query\
                            .filter(and_(
                                GroupPackageListingAcl.grouppackagelistingid
                                    == GroupPackageListing.id,
                                GroupPackageListing.packagelistingid
                    ...
                    for group_acls in changed_acls:
                        for change_acl in group_acls:
                            pkg_listing_log_msg = '%s has set %s to %s' \
                                ...
                                StatusTranslation.query\
                                        .filter_by(
                                            statuscodeid=change_acl\
                                                .statuscode).one().statusname,
                                #pylint:enable-msg=E1101
                                change_acl.grouppackagelisting.groupname,
                                pkg.name,
                                collection.name,
    ...
    def remove_user(self, username, pkg_name, collectn_list=None):
        ...
        if collectn_list:
            ...
            for branchname in collectn_list:
                ...
                pkg_listing = session.query(PackageListing)\
                    .filter_by(packageid=pkg.id, collectionid=collectn.id)\
                    .one()
                #pylint:enable-msg=E1101
        ...
        for pkg_listing in package_listings:
            ...
            for acl in acls:
                person_acl = _create_or_modify_acl(pkg_listing, username,
                        acl.acl, 'Obsolete')
    ...
    def set_critpath(self, pkg_list=None, critpath=True, collctn_list=None,
        ...
        if pkg_list:
            ...
            pkg_listing_ids = select((PackageListingTable.c.id,))\
                              .where(and_(not_(PackageListingTable.c\
                                                   .statuscode\
                                                   .in_((STATUS['EOL'],
                                                         STATUS['Removed']))),
                                          PackageListingTable.c.critpath!=\
